...
jQuery(document).ready(function($){
    ...
    if ($.cookie("user_current_opt") === null) {
        $('.group:first').fadeIn('fast');
        $('#tab-nav li:first').addClass('current');
    } else {
        ...
        $.each(hooks, function(key, value) {
            ...
            if ($.cookie("user_current_opt") == '#of-option-'+ value) {
                $('.group#of-option-' + value).fadeIn();
                $('#tab-nav li.' + value).addClass('current');
            }
    ...
    //Current Menu Class
    $('#tab-nav li a').click(function(evt){
        ...
        $.cookie('user_current_opt', clicked_group, { expires: 7, path: '/' });
    ...
    //Expand Options
    var flip = 0;
    $('#options-to-list').click(function()
    {
        if(flip == 0)
        {
            flip = 1;
            $('#container #tab-nav').hide();
            $('#container #tab-content').width(755);
            $('#container .group').add('#container .group h2').show();

            $(this).removeClass('options-expand');
            $(this).addClass('options-collapse');
            $(this).text('Collapse');
        ...
        {
            flip = 0;
            $('#container #tab-nav').show();
            $('#container #tab-content').width(595);
            $('#container .group').add('#container .group h2').hide();
            $('#container .group:first').show();
            $('#container #tab-nav li').removeClass('current');
            $('#container #tab-nav li:first').addClass('current');

            $(this).removeClass('options-collapse');
            $(this).addClass('options-expand');
            $(this).text('Expand');
        }
    });
        ...
        if (answer){
            ...
            $.post(ajaxurl, data, function(response) {
                if(response==-1){ //failed
                    ...
                    var success_popup = $('#actions-save');
    ...
    /** AJAX Save Options */
    $('#options-save , #options-save-upper').live('click',function() {
        ...
        var serializedReturn = $('#options-form :input[name][name!="security"][name!="options-reset"]').serialize();
